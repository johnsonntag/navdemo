...
void latlon2xy(double lat,double lon,double loncm,int projection,double *x,double *y)
{
    ...
    // Calculate x and y according to selected projection
    if (projection==0)
    {
        geod2ps(lat,lon,70.0*DEG2RAD,315.0*DEG2RAD,1.0,AE,FLAT,x,y);
    }
    else if (projection==1)
    {
        geod2utm(lat,lon,loncm,y,x,&zone);
    }
    else
    {
        geod2ps(lat,lon,-71.0*DEG2RAD,0.0*DEG2RAD,1.0,AE,FLAT,x,y);
    }
